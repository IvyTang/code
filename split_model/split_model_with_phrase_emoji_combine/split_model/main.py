...
def export_graph(session, i, phase="lm"):
    if phase == "lm":
    ...
    if FLAGS.use_emoji_combine:
        variables_to_export.extend(["Online/WordModel/emoji_probabilities",
                                    "Online/WordModel/emoji_top_k_prediction"])
    ...
    model_export_name = os.path.join(model_export_path,
                                     config_name[config_name.rfind("/")+1:] + "-iter" + str(i) + "-" + phase + '.pb')
...
def run_letter_epoch(session, data, word_model, letter_model, cfg, eval_op=None, verbose=False):
    ...
    for step, (epoch_size, lm_data, letter_data, _, _, _) in \
...
def run_word_epoch(session, data, word_model, cfg, lm_phase_id, eval_op=None, verbose=False):
    """Runs the model on the given data."""
    start_time = time.time()
    costs = 0.0
    costs_phrase_p = 0
    ...
    num_word = 0
    fetches = {}
    costs_list = 0.0, 0.0, 0.0, 0.0
    previous_state = session.run(word_model.initial_state)

    for step, (epoch_size, lm_data, _, phrase_p_data, phrase_data, emoji_data) in \
            enumerate(data_feeder.data_iterator(data, cfg)):
        if step >= FLAGS.laptop_discount > 0:
            break
        if step >= epoch_size:
        ...
        feed_dict = {word_model.input_data: lm_data[0],
                     ...
                     word_model.target_emoji_data: emoji_data[0],
                     word_model.target_emoji_mask: emoji_data[1],
                     word_model.initial_state: previous_state
                     }
        # The language model's final states of previous epoch is the language model's initial state of current epoch.
        ...
        phase_list = ["word", "phrase prob", "phrase", "emoji"]
        costs_list = [np.exp(costs / iters), np.exp(costs_phrase_p / iters),
                      np.exp(costs_phrase / iters), np.exp(costs_emoji / iters)]

        if verbose and step % (epoch_size // 100) == 0:

            print("[" + time.strftime('%Y-%m-%d %H:%M:%S') + "] "
                  "%.3f " % (step * 1.0 / epoch_size) + phase_list[lm_phase_id] +
                  " ppl: %.3f speed: %.0f wps" % (costs_list[lm_phase_id], num_word / (time.time() - start_time)))
            sys.stdout.flush()

    return costs_list[lm_phase_id]


def run_test_epoch(session, data, word_model, letter_model, cfg, logfile, phase="lm"):
    """Runs the model on the given data."""
    start_time = time.time()
    fetches = {}
    fetches_letter = {}
    ...
    for step, (epoch_size, lm_data, letter_data, phrase_p_data, phrase_data, emoji_data) in \
            enumerate(data_feeder.data_iterator(data, cfg)):
        if step >= FLAGS.laptop_discount > 0:
            break
        if step >= epoch_size:
        ...
        if phase == "lm":
            ...
            complete_rows = np.arange(letter_batch_size) * cfg.max_word_length + letter_lengths - 1
    ...
    if phase == "lm":
        ...
        print("Language model: Top1 accuracy = {0}, top3 accuracy = {1}, top5 accuracy = {2} ".format(
            top1_acc, top3_acc, top5_acc), file=logfile)
    else:
...
def main(_):
    ...
    cfg = Config()
    cfg.get_config(FLAGS.vocab_path, FLAGS.model_config)

    test_cfg = Config()
    test_cfg.get_config(FLAGS.vocab_path, FLAGS.model_config)
    ...
    with tf.Graph().as_default():
        ...
        gpu_config.gpu_options.per_process_gpu_memory_fraction = cfg.gpu_fraction
        train_data = data_feeder.read_file(FLAGS.data_path, cfg, is_train=True)
        valid_data = data_feeder.read_file(FLAGS.data_path, cfg, is_train=False)
        print("in words vocabulary size = %d\nout words vocabulary size = %d\nin letters vocabulary size = %d"
              "\nphrase vocabulary size = %d\nemojis vocabulary size = %d" %
              (cfg.vocab_size_in, cfg.vocab_size_out, cfg.vocab_size_letter, cfg.vocab_size_phrase, cfg.vocab_size_emoji))

        with tf.Session(config=gpu_config) as session:
            with tf.name_scope("Train"):
                # train model on train data
                with tf.variable_scope("WordModel", reuse=False, initializer=initializer):
                    mtrain_word = WordModel(is_training=True, cfg=cfg)
                    train_word_op = mtrain_word.train_op
                with tf.variable_scope("LetterModel", reuse=False, initializer=initializer):
                    mtrain_letter = LetterModel(is_training=True, cfg=cfg)
            ...
            with tf.name_scope("Valid"):
                ...
                with tf.variable_scope("WordModel", reuse=True, initializer=initializer):
                    mvalid_word = WordModel(is_training=False, cfg=cfg)
                with tf.variable_scope("LetterModel", reuse=True, initializer=initializer):
                    mvalid_letter = LetterModel(is_training=False, cfg=cfg)

            with tf.name_scope("Test"):
                # test model on test data
                with tf.variable_scope("WordModel", reuse=True, initializer=initializer):
                    mtest_word = WordModel(is_training=False, cfg=cfg)
                with tf.variable_scope("LetterModel", reuse=True, initializer=initializer):
                    mtest_letter = LetterModel(is_training=False, cfg=cfg)

            with tf.name_scope("Online"):
                # language model and letter model to be saved and exported, and the batch size is 1.
                with tf.variable_scope("WordModel", reuse=True, initializer=initializer):
                    monline_word = WordModel(is_training=False, cfg=test_cfg)
                with tf.variable_scope("LetterModel", reuse=True, initializer=initializer):
                    monline_letter = LetterModel(is_training=False, cfg=test_cfg)
            ...
            # It contains four phases: lm_train_phase, phrase_prob_train_phase, phrase_train_phase and emoji_train_phase
            # If you don't want to train phrase or emoji, just skip the corresponding phase.

            lm_phase_num = 4
            for lm_phase_id in range(lm_phase_num):

                # ln_phase_id = [0,1,2,3], representing lm_train_phase, phrase_prob_train_phase,
                #                phrase_train_phase and emoji_train_phase
                if not FLAGS.use_phrase and lm_phase_id in [1, 2]:
                    continue
                if not FLAGS.use_emoji_combine and lm_phase_id == 3:
                    continue
                print("lm training phase: %d" % (lm_phase_id + 1), file=logfile)
                if lm_phase_id != 1:
                    max_train_epoch = cfg.max_max_epoch
                else:
                    max_train_epoch = 2
                # the max_train_epoch of phrase_prob_train_phase is 2, it is enough.
                for i in range(max_train_epoch):
                    lr_decay = cfg.lr_decay ** max(i + 1 - cfg.max_epoch, 0)
                    print("lm training phase: %d" % (lm_phase_id + 1))
                    mtrain_word.assign_lr(session, cfg.learning_rate * lr_decay)
                    print(time.strftime('%Y-%m-%d %H:%M:%S'), file=logfile)
                    print("Epoch: %d Learning rate: %.3f" % (i + 1, session.run(mtrain_word.lr)), file=logfile)
                    train_perplexity = run_word_epoch(session, train_data, mtrain_word, cfg, lm_phase_id,
                    ...
                    valid_perplexity = run_word_epoch(session, valid_data, mvalid_word, cfg, lm_phase_id)
                    run_test_epoch(session, valid_data, mtest_word, mtest_letter, cfg, logfile, phase="lm")
            ...
            for i in range(cfg.max_max_epoch):
                ...
                mtrain_letter.assign_lr(session, cfg.learning_rate * lr_decay)
                print(time.strftime('%Y-%m-%d %H:%M:%S'), file=logfile)
                print("Epoch: %d Learning rate: %.3f" % (i + 1, session.run(mtrain_letter.lr)), file=logfile)
                train_perplexity = run_letter_epoch(session, train_data, mtrain_word, mtrain_letter, cfg,
                ...
                print(time.strftime('%Y-%m-%d %H:%M:%S'), file=logfile)
                valid_perplexity = run_letter_epoch(session, valid_data, mtrain_word, mvalid_letter, cfg)
                run_test_epoch(session, valid_data, mtest_word, mtest_letter, cfg, logfile, phase="letter")
