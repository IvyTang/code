...
class WordModel(object):
    ...
    def __init__(self, is_training, cfg):
        ...
        self.sequence_length = tf.placeholder_with_default(
            input=tf.fill(dims=[self.batch_size], value=self.num_steps), shape=[self.batch_size],
        ...
        # phrase_probabilities.shape = [batch_size * num_steps, vocab_size_phrase]
        ...
        loss_phrase_p = tf.contrib.seq2seq.sequence_loss(
            ...
            self.target_phrase_p, self.target_phrase_p_masks, average_across_timesteps=False, average_across_batch=False)
        ...
        loss_phrase = tf.contrib.seq2seq.sequence_loss(
            ...
            self.target_phrase_data, self.target_phrase_data_masks, average_across_timesteps=False, average_across_batch=False)

        self._phrase_p_cost = tf.reduce_sum(loss_phrase_p)

        self._phrase_cost = tf.reduce_sum(loss_phrase)
        ...
        loss_word = tf.contrib.seq2seq.sequence_loss(
            ...
            self.target_data, self.output_masks, average_across_timesteps=False, average_across_batch=False)

        self._cost = tf.reduce_sum(loss_word)
        ...
        grads, _ = tf.clip_by_global_norm(tf.gradients(self._cost, tvars), cfg.max_grad_norm)
        optimizer = tf.train.GradientDescentOptimizer(self._lr)

        self.global_step = tf.contrib.framework.get_or_create_global_step()
        self._train_op = optimizer.apply_gradients(zip(grads, tvars), global_step=self.global_step)
        grads_phrase_p, _ = tf.clip_by_global_norm(tf.gradients(self._phrase_p_cost, tvars_phrase_p), cfg.max_grad_norm)
        ...
        grads_phrase, _ = tf.clip_by_global_norm(tf.gradients(self._phrase_cost, tvars_phrase), cfg.max_grad_norm)
...
class LetterModel(object):
    ...
    def __init__(self, is_training, cfg):
        ...
        if cfg.word_hidden_size != cfg.letter_hidden_size:
            self._initial_state = tf.placeholder_with_default(tf.reshape(tf.matmul(
                tf.reshape(self.lm_state_in, [-1, cfg.word_hidden_size]), lm_state_to_letter_state),
                letter_state_as_tensor_shape), letter_state_as_tensor_shape, name="state")
        else:
