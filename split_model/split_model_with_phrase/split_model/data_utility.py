...
class DataUtility:
    ...
    def data2ids_line(self, data_line):
        ...
        if len(letters_line) != len(words_line):
            return None
        words_ids = self.words2ids(words_line)
        letters_ids = [self.letters2ids(letters.lower()) for letters in letters_line]
    ...
    def sentence2ids(self, sentence):
        ...
        word_letters = " ".join(words_array[-1])
        words_array = words_array[:-1]
        words_ids = self.words2ids(words_array)
        letters_ids = self.letters2ids(word_letters)
