...
def run_test_epoch(session, data, word_model, letter_model, cfg, logfile, phase="lm"):
    ...
    if phase == "lm":
        ...
        print("Language model: Top1 accuracy = {0}, top3 accuracy = {1}, top5 accuracy = {2} ".
              format(top1_acc, top3_acc, top5_acc), file=logfile)
    else:
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        print("in words vocabulary size = %d\nout words vocabulary size = %d\nin letters vocabulary size = %d"
              "\nphrase vocabulary size = %d" % (cfg.vocab_size_in, cfg.vocab_size_out, cfg.vocab_size_letter,
                                                 cfg.vocab_size_phrase))
