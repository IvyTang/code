...
#                if is_training and config.keep_prob < 1:
#                    lemma_inputs = tf.nn.dropout(lemma_inputs, config.keep_prob)
        ...
        with tf.variable_scope("LemmaSoftmax"):
            ...
            lemma_W1 = tf.get_variable(
                "W1",
                shape=[self.hidden_size * 4 + self.embedding_size, 512], dtype=tf.float32,
                initializer=tf.contrib.layers.xavier_initializer())
            lemma_b1 = tf.get_variable("b1", [512], dtype=tf.float32,
                                       initializer=tf.constant_initializer(0.1))
            convert_matrix = tf.get_variable("small_w", shape=[1024, self.embedding_size],  dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
            ...
            lemma_W3 = tf.get_variable(
                ...
                initializer=tf.contrib.layers.xavier_initializer())
            lemma_b3 = tf.get_variable("b3", [self.vocab_size_out], dtype=tf.float32,
                                       initializer=tf.constant_initializer(0.1))
            ...
            att_logits = tf.nn.xw_plus_b(tf.matmul(att_output_2,convert_matrix),
                                         lemma_W3, lemma_b3, name="scores")
        l2_loss = tf.nn.l2_loss(lemma_W1) + tf.nn.l2_loss(lemma_b1) + tf.nn.l2_loss(lemma_W2) + tf.nn.l2_loss(lemma_b2) + tf.nn.l2_loss(lemma_W3) + tf.nn.l2_loss(lemma_b3)
        ...
        self._att_cost = att_cost = tf.reduce_sum(att_loss) + 0.001 * l2_loss
    ...
    def __init__(
        ...
        with tf.variable_scope("CNN"):
            ...
            with tf.variable_scope("output"):
                ...
                W = tf.get_variable(
                    "W",
                    shape=[config.letter_embedding_size * 2, num_classes], dtype=tf.float32,
                    initializer=tf.contrib.layers.xavier_initializer())
