...
flags.DEFINE_string("data_path", "resource/trace_data_sample_old/", "Where the training/test data is stored.")
flags.DEFINE_string("vocab_path", "resource/trace_data_sample_old/", "Where the training/test data is stored.")
flags.DEFINE_string("file_name_stem", "ptb", "corpus file name (without suffix).")
flags.DEFINE_string("save_path", "target/model_with_bucket/", "Model output directory.")
flags.DEFINE_string("model_name", "model_test", "Pick a name for your model.")
flags.DEFINE_string("graph_save_path", "target/graph_with_bucket/", "Exported graph directory.")
...
def run_evaluate_epoch(session, model, logfile, config, word_dict=None, data_feeder=None, eval_op=None, output_limit=10000):
    ...
    prediction_made = 0.0
    prediction_made_last = 0.0
    top1_correct_total, top3_correct_total, top5_correct_total = 0.0, 0.0, 0.0
    top1_last_correct_total, top3_last_correct_total, top5_last_correct_total = 0.0, 0.0, 0.0
    sum_top1_input_letter_num, sum_top1_word_letters_num = 0, 0
    sum_top3_input_letter_num, sum_top3_word_letters_num = 0, 0

    for step in range(epoch_size):
        inputs, outputs, masks, lengths, batch_length, words_num, letters_num = data_feeder.next_batch_fixmask(batch_size)
        model_with_bucket = model[buckets.index(batch_length)]
        fetches["cost"] = model_with_bucket.cost
        fetches["top_k_prediction"] = model_with_bucket.top_k_prediction
        if eval_op is not None:
            eval_op_with_bucket = eval_op[buckets.index(batch_length)]
            fetches["eval_op"] = eval_op_with_bucket

        feed_dict = {model_with_bucket.input_data: inputs,
                     model_with_bucket.target_data: outputs,
                     model_with_bucket.output_masks: masks,
                     # model_with_bucket.sequence_length: lengths,
                     model_with_bucket.top_k: num_top}
        ...
        masks_as_a_column = masks.reshape([-1])
        masks_as_a_column_last = np.copy(masks_as_a_column)
        for i in range(len(masks_as_a_column)):
            if masks_as_a_column[i] > 0:
                masks_as_a_column[i] = 1
            if masks_as_a_column_last[i] > 1:
                masks_as_a_column_last[i] = 1
            else:
                masks_as_a_column_last[i] = 0

        top1_correct = top_k_prediction[:, 0] == y_as_a_column
        top3_correct = top1_correct | (top_k_prediction[:, 1] == y_as_a_column) | (top_k_prediction[:, 2] == y_as_a_column)
        top5_correct = top3_correct | (top_k_prediction[:, 3] == y_as_a_column) | (top_k_prediction[:, 4] == y_as_a_column)

        top1_correct_num = np.sum((top1_correct * masks_as_a_column).astype(float))
        top3_correct_num = np.sum((top3_correct * masks_as_a_column).astype(float))
        top5_correct_num = np.sum((top5_correct * masks_as_a_column).astype(float))
        top1_correct_total += top1_correct_num
        top3_correct_total += top3_correct_num
        top5_correct_total += top5_correct_num

        top1_last_correct_num = np.sum((top1_correct * masks_as_a_column_last).astype(float))
        top3_last_correct_num = np.sum((top3_correct * masks_as_a_column_last).astype(float))
        top5_last_correct_num = np.sum((top5_correct * masks_as_a_column_last).astype(float))
        top1_last_correct_total += top1_last_correct_num
        top3_last_correct_total += top3_last_correct_num
        top5_last_correct_total += top5_last_correct_num

        if prediction_made < output_limit:
            if word_dict is not None:
                for i in range(len(y_as_a_column)):
                    if (masks_as_a_column[i] > 0):
        ...
        prediction_made += np.sum(masks_as_a_column)
        prediction_made_last += np.sum(masks_as_a_column_last)

        for i in range(batch_size):
            words_num_one = words_num[i]
            letters_num_one = letters_num[i]
            mask_line = masks[i]
            top_k_prediction_reshape = top_k_prediction.reshape([batch_size, len(mask_line), num_top])
            top_k_prediction_line = top_k_prediction_reshape[i]
            output_line = outputs[i]

            input_letter_num_top1 = 0
            if words_num_one == 0:
                input_letter_num_top1 = 1
            word_letter_num_top1 = letters_num_one
            for j in range(len(mask_line)):
                if (mask_line[j] > 0):
                    if (top_k_prediction_line[j][0] == output_line[j]):
                        break
                    input_letter_num_top1 += 1
            if (input_letter_num_top1 == word_letter_num_top1 + 1):
                input_letter_num_top1 -= 1
                word_letter_num_top1 = 0
            sum_top1_input_letter_num += input_letter_num_top1
            sum_top1_word_letters_num += word_letter_num_top1

            input_letter_num_top3 = 0
            if words_num_one == 0:
                input_letter_num_top3 = 1
            word_letter_num_top3 = letters_num_one
            for j in range(len(mask_line)):
                if (mask_line[j] > 0):
                    if top_k_prediction_line[j][0] == output_line[j] or top_k_prediction_line[j][1] == output_line[j] or top_k_prediction_line[j][2] == output_line[j]:
                        break
                    input_letter_num_top3 += 1
            if (input_letter_num_top3 == word_letter_num_top3 + 1):
                input_letter_num_top3 -= 1
                word_letter_num_top3 = 0
            sum_top1_input_letter_num += input_letter_num_top3
            sum_top1_word_letters_num += word_letter_num_top3

            # print (words_num_one)
            # print (letters_num_one)
            # print (mask_line)
            # print (top_k_prediction_line)
            # print(output_line)
            # print (input_letter_num_top1)
            # print (word_letter_num_top1)
            # print(input_letter_num_top3)
            # print(word_letter_num_top3)

            sum_top1_input_letter_num += input_letter_num_top1
            sum_top1_word_letters_num += word_letter_num_top1
            sum_top3_input_letter_num += input_letter_num_top3
            sum_top3_word_letters_num += word_letter_num_top3
    ...
    print("Top1 accuracy = {0}, top3 accuracy = {1}, top5 accuracy = {2} ".format(top1_correct_total / prediction_made,
          ...
          file=logfile)
    print("Top1 last accuracy = {0}, top3 last accuracy = {1}, top5 last accuracy = {2} ".format(
        top1_last_correct_total / prediction_made_last, top3_last_correct_total / prediction_made_last,
        top5_last_correct_total / prediction_made_last), file=logfile)
    print("Top1 accuracy = {0}, top3 accuracy = {1}, top5 accuracy = {2} ".format(top1_correct_total / prediction_made,
                                                                                  top3_correct_total / prediction_made,
                                                                                  top5_correct_total / prediction_made))
    print("Top1 last accuracy = {0}, top3 last accuracy = {1}, top5 last accuracy = {2} ".format(
        top1_last_correct_total / prediction_made_last, top3_last_correct_total / prediction_made_last,
        top5_last_correct_total / prediction_made_last))

    print ("Top1 input efficiency = {0}, top3 input efficiency = {1}".format(sum_top1_word_letters_num / sum_top1_input_letter_num, sum_top3_word_letters_num / sum_top3_input_letter_num))
...
def run_epoch(session, model, config, eval_op=None, data_feeder=None, verbose=False):
    """Runs the model on the given data."""
    start_time = time.time()
    costs = 0.0
    iters = 0
    ...
    for step in range(epoch_size):
        if step >= epoch_size // FLAGS.laptop_discount:  # Early stopping for debug purpose
            break
        inputs, outputs, masks, lengths, batch_length, _, _ = data_feeder.next_batch_fixmask(batch_size)
        # print(batch_length)
        model_with_bucket = model[buckets.index(batch_length)]
        fetches["cost"] = model_with_bucket.cost
        if eval_op is not None:
...
def learn_sparse_softmax(session, model, verbose=False):
    ...
    for i in range(model.basis_size):
        ...
        indices = np.r_[np.array([i], dtype=np_index_data_type()),
                        np.array([key for key in random_basis.keys()], dtype=np_index_data_type())]
...
def main(_):
    ...
    if not os.path.isdir(FLAGS.graph_save_path):
        os.makedirs(FLAGS.graph_save_path)
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=gpu_config) as session:
            with tf.name_scope("Train"):
                mtrain = []
                train_op_array = []
                train_feeder = DataFeederContext(config, vocab_file_in_words=vocab_file_in_words,
                ...
                for j, bucket in enumerate(buckets):
                    with tf.variable_scope("Model", reuse=True if j > 0 else None, initializer=initializer):
                        mtrain.append(PTBModel(is_training=True, config=config, bucket=bucket))
                        train_op_array.append(mtrain[j].train_op)
                        tf.summary.scalar("Training Loss_%d" % bucket, mtrain[j].cost)
                tf.summary.scalar("Learning Rate", mtrain[0].lr)

            with tf.name_scope("Valid"):
                mvalid = []
                valid_feeder = DataFeederContext(config, vocab_file_in_words=vocab_file_in_words,
                                                 vocab_file_in_letters=vocab_file_in_letters,
                                                 vocab_file_out=vocab_file_out,
                ...
                for j, bucket in enumerate(buckets):
                    with tf.variable_scope("Model", reuse=True, initializer=initializer):
                        mvalid.append(PTBModel(is_training=False, config=config, bucket=bucket))
                        tf.summary.scalar("Validation Loss_%d" % bucket, mvalid[j].cost)

            # Evaluate on test data
            with tf.name_scope("Test"):
                mtest = []
                test_feeder = DataFeederContext(config, vocab_file_in_words=vocab_file_in_words,
                                                vocab_file_in_letters=vocab_file_in_letters,
                                                vocab_file_out=vocab_file_out,
                                                corpus_file_in_words=dev_file_in_words,
                                                corpus_file_in_letters=dev_file_in_letters,
                ...
                for j, bucket in enumerate(buckets):
                    with tf.variable_scope("Model", reuse=True, initializer=initializer):
                        mtest.append(PTBModel(is_training=False, config=config,  bucket=bucket))
            ...
            with tf.name_scope("Online"):
                with tf.variable_scope("Model", reuse=True, initializer=initializer):
                    monline = PTBModel(is_training=False, config=test_config, bucket=1)
            ...
            if FLAGS.mode == "pretrain":
                ...
                for i in range(config.max_max_epoch // FLAGS.laptop_discount):
                    ...
                    for j, bucket in enumerate(buckets):
                        ...
                        # print("Epoch: %d, Learning_rate for model %d, %.3f" % (i + 1, j, session.run(mtrain[j].lr)))
                    print(time.strftime('%Y-%m-%d %H:%M:%S'), file=logfile)
                    print("Epoch: %d Learning rate: %.3f" % (i + 1, session.run(mtrain[0].lr)), file=logfile)
                    train_perplexity = run_epoch(session, mtrain, config, eval_op=train_op_array, data_feeder=train_feeder, verbose=True)

                    print(time.strftime('%Y-%m-%d %H:%M:%S'), file=logfile)
                    print("Epoch: %d Train Perplexity: %.3f" % (i + 1, train_perplexity), file=logfile)
                    logfile.flush()

                    valid_perplexity = run_epoch(session, mvalid, config, data_feeder=valid_feeder)
                    print(time.strftime('%Y-%m-%d %H:%M:%S'), file=logfile)
                ...
                test_perplexity = run_evaluate_epoch(session, mtest, logfile, config, word_dict=data_utility.id2token_out, data_feeder=test_feeder)
                ...
                learn_sparse_embedding(session, mtrain[0], verbose=True)
                ...
                learn_sparse_softmax(session, mtrain[0], verbose=True)
                ...
                for i in range(config.finetune_epoch // FLAGS.laptop_discount):
                    ...
                    for j in range(len(buckets)):
                        mtrain[j].assign_lr(session, config.finetune_learning_rate * lr_decay)

                    print(time.strftime('%Y-%m-%d %H:%M:%S'), file=logfile)
                    print("Epoch: %d Learning rate: %.3f" % (i + 1, session.run(mtrain[0].lr)), file=logfile)
                    train_perplexity = run_epoch(session, mtrain, config, eval_op=train_op_array, data_feeder=train_feeder,
                                                 verbose=True)
